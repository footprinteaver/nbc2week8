        ...
        public Monster(int currentStage)  // 몬스터 생성자
        {
            ...
            switch (randMonster)
            {
                ...
                case (int)MonsterType.KimHyunjeong:
                    MonsterSetting("김현정 튜터", 60, 10, 7, 10,400,10);
        ...
        public Item DropItem(int type)          //몬스터 별 드롭 아이템 설정
        {
            ...
            switch (type)
            {
                case (int)MonsterType.LeeHanSol:
                    ...
                    {
                        new Item("핸드백", "작은 크기지만 그 무엇보다 많은게 들어있습니다", Item.ItemType.Weopon, 4, 0, 0),
                        new Item("프랜치 코트", "방어구보단 패션 아이템 같습니다", Item.ItemType.Armor, 0, 1, 0),
                        new Item("상처치료연고", "체력 10 회복", Item.ItemType.Potion, 0, 0, 10)
                    };
                ...
                case (int)MonsterType.MonYeongOh:
                    monsterItem = new List<Item>
                    {
                        new Item("코딩 책", "사전에 비견되는 딱딱함과 묵직함을 지녔습니다", Item.ItemType.Weopon, 7, 0, 0),
                        new Item("가죽 자켓", "좋은 브랜드라 약간의 방어력을 기대해도 될 것 같습니다", Item.ItemType.Armor, 0, 3, 0),
                        new Item("압박붕대", "체력 15 회복", Item.ItemType.Potion, 0, 0, 15)
                    };
                    monsterDropRate = new List<int>
                    {
                        15,
                ...
                case (int)MonsterType.HanHyoseung:
                    monsterItem = new List<Item>
                    {
                        new Item("마우스", "\"딸깍\"", Item.ItemType.Weopon, 10, 0, 0),
                        new Item("롱패딩", "전신을 감싸지만 실상은 얇은 재질입니다", Item.ItemType.Armor, 0, 4, 0),
                        new Item("봉합술 키트", "체력 25 회복", Item.ItemType.Potion, 0, 0, 25)
                    };
                    monsterDropRate = new List<int>
                    {
                        10,
                ...
                case (int)MonsterType.LeeHanbyeol:
                    monsterItem = new List<Item>
                    {
                        new Item("고장난 키보드", "무기로 쓰기에 적합한 키보드", Item.ItemType.Weopon, 15, 0, 0),
                        new Item("발가락 수면양말", "굉장히 편안한 수면양말", Item.ItemType.Armor, 0, 7, 0),
                        new Item("봉합술 키트", "체력 25 회복", Item.ItemType.Potion, 0, 0, 25)
                    };
                    monsterDropRate = new List<int>
                    {
                ...
                case (int)MonsterType.KimYeongHo:
                    monsterItem = new List<Item>
                    {
                        new Item("커피 보틀", "모서리에 맞으면 아픈 보틀", Item.ItemType.Weopon, 9, 0, 0),
                        new Item("고장난 키보드", "무기로 쓰기에 적합한 키보드", Item.ItemType.Weopon, 15, 0, 0),
                        new Item("봉합술 키트", "체력 25 회복", Item.ItemType.Potion, 0, 0, 25)
                    };
                    monsterDropRate = new List<int>
